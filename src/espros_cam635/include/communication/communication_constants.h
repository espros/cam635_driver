...
namespace CommunicationConstants
{
  ...
  namespace CommandList
  {
    ...
    const uint8_t COMMAND_SET_FILTER_SINGLE_SPOT = 0x0F;                       ///<Command to set the temporal filter for the single spot
     const uint8_t COMMAND_SET_INTERFERENCE_DETECTION = 0x11;                   ///<Command to set the interference detection settings
  ...
  const uint8_t COMMAND_GET_INTEGRATION_TIME_3D = 0x27;                      ///<Command to read the integration time. Important when using automatic mode
    ...
    const uint8_t COMMAND_SET_ILLUMINATION_POWER = 0x6C;                       ///<Command to set the illumination power
  }
  namespace Update
  {
  ...
  namespace Identification
  {
    ...
    const uint8_t DEVICE_TOFCC = 0x03;                                             ///<Device value for a Tof Controller
    ...
    const uint32_t DEVICE_TOFCC635  = (CHIP_TYPE_EPC635 << 8) | DEVICE_TOFCC;  ///<Combination of chip type and device to get TofCC635
  ...
  namespace InterferenceDetection
  {
    const uint32_t INDEX_ENABLED = 2;
    const uint32_t INDEX_USE_LAST_VALUE = 3;
    const uint32_t INDEX_LIMIT = 4;
  }
